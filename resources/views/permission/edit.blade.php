        ...
        <div class="post d-flex flex-column-fluid" id="kt_post">
            <div id="kt_content_container" class="container-xxl">
                <form id="permission_form" class="form d-flex flex-column flex-lg-row" method="POST"
                      action="{{ route('permission.update', $permission->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                                ...
                                <div class="mb-10 fv-row">
                                    ...
                                    <select class="form-select mb-2" data-control="select2" name="permission_group_id"
                                        ...
                                        @foreach ($permission_group as $item)
                                            <option
                                                value="{{ $item->id }}" @selected(old('permission_group_id', $permission->permission_group_id) == $item->id)>
                                                {{ $item->name }}</option>
                                        @endforeach
                                    </select>
